...
def build_module_pool(
    ...
    Объединяет:
    1. Равномерную сетку от min_m до max_m с шагом step (если default_only=False)
    ...
    Args:
        ...
        default_only: Если True, использовать только ГОСТ и DP модули (без сетки)
    ...
    # 1. Равномерная сетка (только если не default_only)
    if not default_only:
